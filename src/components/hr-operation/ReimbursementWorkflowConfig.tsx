...
import { Save, AlertCircle, RefreshCw, Users } from "lucide-react";
...
type Workflow = {
    id?: number;
    employee_wallet_id: number | null;
...
export default function ReimbursementWorkflowConfig() {
    ...
    const [employeeWallets, setEmployeeWallets] = useState<any[]>([]);
    ...
    useEffect(() => {
        fetchWorkflows();
        fetchEmployeeWallets();
    }, []);

    const fetchEmployeeWallets = async () => {
        setLoadingWallets(true);
        try {
            const data = await apiClient<any>(
                "/reimbursements/wallets",
                { withAuth: true }
            );
            const list = Array.isArray(data) ? data : (data?.data || []);
            setEmployeeWallets(list);
        } catch (error) {
            console.error("Failed to fetch employee wallets:", error);
            showError("Failed to load employee wallets");
        } finally {
    ...
    const fetchWorkflows = async () => {
        ...
        try {
            ...
            const expenseData = await apiClient<{ workflow: Workflow | null }>(
                `/reimbursement-workflow/expense/${walletId}`,
                { withAuth: true }
            );
            if (expenseData.workflow) {
                ...
                setExpenseWorkflow({
                    employee_wallet_id: selectedWalletId,
                    workflow_name: "Expense Approval",
                    workflow_type: "expense",
                    is_common: false,
                    workflow_definition: { workflowName: "Expense Approval", levels: [] },
            ...
            const budgetData = await apiClient<{ workflow: Workflow | null }>(
                `/reimbursement-workflow/budget/${walletId}`,
                { withAuth: true }
            );
    ...
    const handleSaveExpense = async () => {
        ...
        try {
            const walletId = selectedWalletId || 0;
            await apiClient(`/reimbursement-workflow/expense/${walletId}`, {
                method: "POST",
                withAuth: true,
                body: {
    ...
    const handleSaveBudget = async () => {
        ...
        try {
            const walletId = selectedWalletId || 0;
            await apiClient(`/reimbursement-workflow/budget/${walletId}`, {
                method: "POST",
                withAuth: true,
                body: {
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                    <div className="mb-6 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-600">Configure multi-level approval workflow for reimbursement submissions.</p>
                        </div>
                        <div className="flex items-center gap-2">
                            {!isEditingExpense ? (
                                <button
                    ...
                    <div className="mb-6">
                        <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                            ...
                            />
                            <div>
                                <div className="font-medium text-gray-900">Use Common Workflow for All Employees</div>
                                <div className="text-sm text-gray-600">Apply this workflow to all employee reimbursement accounts</div>
                            </div>
                        </label>
                    </div>
                    ...
                    {!expenseWorkflow.is_common && (
                        <div className="mb-6">
                            <h3 className="text-sm font-semibold text-gray-900 mb-3">Select Employee Wallet</h3>
                            {loadingWallets ? (
                                ...
                                <div className="p-4 text-center bg-gray-50 rounded-lg border border-gray-200">
                                    <p className="text-gray-600">No employee wallets found. Please set up a wallet first.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    {employeeWallets.map((wallet, index) => (
                                        <button
                                            key={wallet.id || index}
                                        ...
                                        >
                                            <div className="font-semibold text-gray-900 mb-1">{wallet.employee_name || `Employee #${wallet.employee_id}`}</div>
                                            <div className="text-sm text-gray-600 mb-2">
                                                Dept: {wallet.department_name || 'N/A'}
                                            </div>
                            ...
                            {selectedWalletId && (
                                <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                    <p className="text-sm text-blue-700">
                                        ...
                                        <span className="font-semibold">
                                            {employeeWallets.find(w => w.id === selectedWalletId)?.employee_name}
                                        </span>
                                    </p>
                                </div>
                            )}
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                    <div className="mb-6 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-600">Configure multi-level approval workflow for budget requests.</p>
                        </div>
                        <div className="flex items-center gap-2">
                            {!isEditingBudget ? (
                                <button
                    ...
                    {!budgetWorkflow.is_common && !useSameAsExpense && (
                        <div className="mb-6">
                            ...
                            {loadingWallets ? (
                                ...
                                <div className="p-4 text-center bg-gray-50 rounded-lg border border-gray-200">
                                    <p className="text-gray-600">No wallets found. Please set up a wallet first.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    {employeeWallets.map((wallet, index) => (
