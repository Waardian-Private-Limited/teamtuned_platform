...
export default function LaborAttendanceList() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const [cons, cats] = await Promise.all([
                    apiClient<any>("/labor/contractors", { withAuth: true }),
                    apiClient<any>("/labor/categories", { withAuth: true })
                ]);
                                    ...
                                    laborers.map((emp) => {
                                        ...
                                        return (
                                            <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    {isPresent ? (
                                                        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${isWorking
                                                            ? 'bg-blue-50 text-blue-700 border border-blue-100'
                                                            : 'bg-green-50 text-green-700 border border-green-100'
                                                            }`}>
                                                            {isWorking ? 'Working Now' : 'Completed'}
                                                        </span>
                                                    ) : (
