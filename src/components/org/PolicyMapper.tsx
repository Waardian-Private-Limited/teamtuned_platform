...
export default function PolicyMapper() {
    ...
    // Bulk Update State
    const [targetPolicy, setTargetPolicy] = useState<number | 'no_change'>('no_change');
    ...
    useEffect(() => {
        const fetchMetadata = async () => {
            try {
                const [deptRes, roleRes, siteRes, policyRes] = await Promise.all([
                    apiClient<any>('/organization/departments'),
                    apiClient<any>('/organization/roles'),
                    apiClient<any>('/sites'),
                    apiClient<any>('/policy-mapper/policies'),
                ]);
                ...
                const getArray = (res: any, key?: string) => {
                    ...
                    if (res.policies && Array.isArray(res.policies)) return res.policies;
                    return [];
                };
                ...
                setPolicies(getArray(policyRes, 'policies'));
            } catch (error) {
    ...
    const fetchEmployees = useCallback(async () => {
        ...
        try {
            ...
            const res = await apiClient<any>('/policy-mapper/employees', { params });
    ...
    const handleApply = async () => {
        ...
        if (targetPolicy === 'no_change') {
            toast('Select a Policy to assign', { icon: 'ℹ️' });
            return;
        }
        ...
        try {
            const payload = {
                ...
                attendance_policy_id: targetPolicy
            };

            const res = await apiClient<{ success: boolean; message: string }>('/policy-mapper/bulk-assign', {
                method: 'POST',
                body: payload
            });
            ...
            <div className="p-4 border-b border-gray-200 bg-gray-50/50 space-y-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs text-gray-500">Bulk assign attendance policies</p>
                    </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-50 sticky top-0 z-10 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                        <tr>
                            ...
                            <th className="p-3 border-b border-gray-200 hidden md:table-cell">Department</th>
                            <th className="p-3 border-b border-gray-200 hidden md:table-cell">Role</th>
                            <th className="p-3 border-b border-gray-200">Current Policy</th>
                        </tr>
                    </thead>
                                    ...
                                    >
                                        ...
                                        <td className="p-3">
                                            {emp.policy_name ? (
                                                ...
                                                <span className="text-xs text-gray-400 italic">No Policy</span>
                                            )}
                                        </td>
                                    </tr>
                                );
                    ...
                    >
                        ...
                        <div className="flex flex-1 items-center gap-2 w-full md:w-auto overflow-x-auto">
                            ...
                            <div className="flex flex-col w-full md:w-64">
                                <label className="text-[10px] uppercase text-gray-500 font-bold mb-0.5">Assign Policy</label>
                                <select
                                    className="text-sm border border-gray-300 rounded-lg px-2 py-1.5 focus:ring-2 focus:ring-blue-500 outline-none w-full"
                                    value={targetPolicy}
                                    onChange={e => setTargetPolicy(e.target.value === 'no_change' ? 'no_change' : Number(e.target.value))}
                                >
                                    <option value="no_change">Select Policy...</option>
                                    {policies.map(p => (
                        ...
                        <button
                            onClick={handleApply}
                            disabled={applying || targetPolicy === 'no_change'}
