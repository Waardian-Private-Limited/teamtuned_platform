            ...
            {loading ? (
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                        ...
                        <div className="grid grid-cols-[1.2fr_1.5fr_1fr_1fr_0.6fr_0.6fr] gap-4 font-bold text-xs text-gray-400 uppercase">
                            <div>Floor Level</div>
                            <div>Purpose</div>
                            <div>Target Date</div>
                        ...
                        {monthlyAchieved.map((item) => (
                            <div key={item.id} className="grid grid-cols-[1.2fr_1.5fr_1fr_1fr_0.6fr_0.6fr] gap-4 items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                                <span className="font-bold flex flex-col">
                                    <span className="text-[10px] text-gray-400 font-medium">
                                        {schedule?.towers?.find((t: any) => t.id === item.towerId)?.name || schedule?.areas?.find((a: any) => a.name === item.towerId)?.name || item.towerId || 'Location'}
                                    </span>
                                    {item.floor === 'Other' ? item.customFloor : item.floor}
                                </span>
                                ...
                                <div className="flex justify-center">
                                    <input
                                        ...
                                        onChange={e => updateArrayField(setMonthlyAchieved, monthlyAchieved, item.id, 'is_achieved', e.target.checked)}
                        ...
                        {equipmentsActual.map((eq) => (
                            <div key={eq.id} className="grid grid-cols-[1.5fr_1fr_100px_1fr] gap-4 items-center bg-gray-50 p-4 rounded-xl border border-gray-100">
                                ...
                                <input
                                    ...
                                    onChange={e => updateArrayField(setEquipmentsActual, equipmentsActual, eq.id, 'breakdown', e.target.value)}
                                    className="p-2 bg-white border border-gray-200 rounded-lg outline-none text-sm"
                                />
                            </div>
                        ))}
                        {equipmentsActual.length === 0 && (
                            <div className="p-10 text-center text-gray-400 font-medium italic">No equipment planned for this date.</div>
                        )}
                    </div>
