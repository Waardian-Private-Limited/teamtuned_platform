...
export default function EmployeeSidebar({
  ...
  // MoM Permissions
  const showMoM = hasFeature('MOM_FEATURE');
  const canCreateMeeting = isOrgAdmin || hasPerm('MOM_MEETING_CREATE');
  const canViewMeetings = isOrgAdmin || hasAnyPerm(['MOM_MEETING_VIEW', 'MOM_MEETING_CREATE']);
                ...
                <div className={`space-y-1 ${isCollapsed ? "" : "pl-0"}`}>
                  <div className={`relative ${isCollapsed ? "" : "ml-3 pl-3 border-l border-gray-100"}`}>
                    <Item
                      ...
                      active={pathname?.startsWith("/employee/meetings") || false}
                    />
                    <Item
                      icon={Settings}
                      label="MoM Config"
                      href="/employee/meetings/config"
                      active={pathname === "/employee/meetings/config"}
                    />
                    <Item
                      icon={ListChecks}
                      label="My Action Items"
                      href="/employee/my-action-items"
                      active={pathname?.startsWith("/employee/my-action-items") || false}
                    />
                  </div>
                </div>
              )}
