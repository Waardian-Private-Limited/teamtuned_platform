...
import type { RoleStatusFilter } from '../../constants/roles.constants';
...
export function RolesEmptyState({
  ...
  const isDeptFiltered = Boolean(hasDepartmentFilter || departmentName);
  const isFiltered = isSearchFiltered || isStatusFiltered || isDeptFiltered;

  let title = 'Create your first role';
  let description = 'Roles bundle permissions so you can assign the same access to many employees at once.';
  let buttonLabel = 'New Role';

  if (isSearchFiltered) {
    title = 'No matching roles';
    description = `No roles match "${searchTerm}". Try adjusting your search query.`;
    buttonLabel = 'Clear search';
  } else if (isDeptFiltered) {
    title = 'No roles in this department';
    description = departmentName
      ? `There are currently no roles assigned to ${departmentName}.`
      : 'There are currently no roles assigned to the selected department.';
    buttonLabel = 'Clear filters';
  } else if (isStatusFiltered) {
    title = status === 'active' ? 'No active roles' : 'No inactive roles';
    description =
      status === 'active'
        ? 'There are currently no active roles in your organization.'
        : 'There are currently no inactive roles in your organization.';
    buttonLabel = 'Clear filters';
  }
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center p-4 sm:p-6 lg:p-8 2xl:p-12 text-center transition-opacity duration-150">
      ...
      {/* Action Button: Create New Role ONLY if unfiltered; otherwise Clear Filters */}
      {!isFiltered ? (
        canAdd && onAdd && (
          <button
            type="button"
