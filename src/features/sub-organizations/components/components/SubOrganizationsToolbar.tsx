...
import { SUB_ORG_STATUS_FILTER_OPTIONS, type SubOrgStatusFilter } from '../../constants/sub-organizations.constants';

interface SubOrganizationsToolbarProps {
  total: number;
  searchInput: string;
  onSearchChange: (value: string) => void;
  status: SubOrgStatusFilter;
  onStatusChange: (value: SubOrgStatusFilter) => void;
  canAdd: boolean;
  onCreate: () => void;
}

export function SubOrganizationsToolbar({
  total,
  searchInput,
  onSearchChange,
  status,
    ...
    <div className="flex flex-col gap-2.5 rounded-xl border border-line bg-surface p-3 sm:p-3.5 lg:flex-row lg:items-center lg:justify-between lg:gap-4 2xl:p-4">
      <div className="flex items-center justify-between gap-3 lg:justify-start">
        <div className="flex items-center gap-2 sm:gap-2.5">
          <h1 className="text-base font-bold tracking-tight text-fg sm:text-lg 2xl:text-xl">Sub Organizations</h1>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2.5">
        ...
        <div className="w-full sm:w-44 md:w-48 lg:w-44 xl:w-48 2xl:w-56">
          <SegmentedControl options={SUB_ORG_STATUS_FILTER_OPTIONS} value={status} onChange={onStatusChange} />
        </div>
        ...
        {canAdd && (
          ...
          >
            ...
            <span className="hidden md:inline">Add Sub-Organization</span>
